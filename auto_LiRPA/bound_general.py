...
class BoundedModule(nn.Module):
    def __init__(self, model, global_input, bound_opts={}, device='cpu', verbose=False):
    ...
    def state_dict(self, destination=None, prefix='', keep_vars=False):
        ...
        for k, v in ori_dict.items():
            if k in self.node_name_map:
                new_dict[prefix + self.node_name_map[k]] = v
            # #  BoundedModule is a submodule
            # elif len(k.split('.')) > 2:
            #     sub_k = k[k.find('.')+1:]
            #     if sub_k in self.node_name_map:
            #         new_dict[k.replace(sub_k, self.node_name_map[sub_k])] = v
    ...
    def compute_bounds(self, x=None, aux=None, C=None, IBP=False, forward=False, method='backward', bound_lower=True,
        ...
        for i in self._modules.values(): # for all nodes
            if i.from_input and hasattr(i, 'nonlinear') and i.nonlinear:  # if node.nonlinear
    ...
    def _IBP_general(self, node=None, C=None):
        ...
        if isinstance(node.upper, torch.Size):
            node.upper = torch.tensor(node.lower)            
        node.interval = (node.lower, node.upper)
    ...
    def _backward_general(self, C=None, node=None, root=None, bound_lower=True, bound_upper=True, return_A=False, average_A=False):
        ...
        while len(queue) > 0:
            ...
            if l.lA is not None or l.uA is not None:
                ...
                # def concretize_fixed_node(node, A):
                #     if A is None:
                #         return 0
                #     A = A.view(A.shape[0], A.shape[1], -1)
                #     if node.from_input:
                #         return A.matmul(node.forward_value.view(A.shape[0], -1, 1)).squeeze(-1)
                #     else:
                #         return A.matmul(node.forward_value.view(-1, 1))

                # if not l.perturbed:
                #     lb = lb + concretize_fixed_node(node, node.lA)
                #     ub = ub + concretize_fixed_node(node, node.uA)
                #     continue

                # if l.from_input:
                #     if l.name in ref:
                #         assert(torch.allclose(torch.tensor(ref[l.name]['lA']), l.lA.transpose(0, 1)))
                #     else:
                #         logger.warning('{} is not in ref'.format(l.name))
                ...
                try:
                    ...
                    if isinstance(lower_b, torch.Tensor):
                        logger.debug('lower_b {}'.format(lower_b.shape))    
                        assert lower_b.shape == (output_dim, batch_size)          
                    if isinstance(upper_b, torch.Tensor):
                        logger.debug('upper_b {}'.format(upper_b.shape))               
                        assert upper_b.shape == (output_dim, batch_size)          
                except:
                ...
                for i, l_pre in enumerate(l.input_name):
                    logger.debug('  {} -> {}'.format(l.name, l_pre))
                    _l = self._modules[l_pre]
